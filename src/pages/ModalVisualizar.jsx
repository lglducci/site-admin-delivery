...
export default function ModalVisualizar({ open, onClose, numero, idEmpresa }) {
  ...
  // Configura impressão somente da modal
  const handlePrint = useReactToPrint({
    content: () => printRef.current,
    documentTitle: `Pedido_${numero}`,
  });
  ...
  useEffect(() => {
    ...
    const fetchItens = async () => {
      setLoading(true);
      try {
        const url = `https://webhook.lglducci.com.br/webhook/visualizarcozinha?numero=${encodeURIComponent(numero)}&id_empresa=${encodeURIComponent(idEmpresa)}`;
